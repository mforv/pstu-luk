...
class ScenarioController(QtCore.QThread):
    ...
    def __init__(self, temp=60, critical_temp=200):
    ...
    def execute_scenario(self, scenario): # последовательно выполнить все действия, если выполнены их условия, в сценарии scenario
        for action in scenario.keys():
            self.action_handler.append([scenario[action], 0])
        self.active = True
        while self.active:
            for action in self.action_handler:
                if action[0][1] is not None and callable(action[0][1]):                             # если действие повторяется, пока выполняется условие
                    if action[0][1]() and (self.current_time - self.start_time) >= action[0][2]:    # если условие выполнено и при этом прошло достаточно времени для повтора
...
if __name__ == '__main__':
    ...
    scenario = {
        ...
        #"старт_сценария": (lambda: sc.set_temp(20.7), None, 0),
        "сс_при_критическом_т": (lambda: elems['сс'].set_state(1, lambda: sc.check('Т > Ткрит', lambda: sc.temp > sc.crit_t)), lambda: elems['сс'].state != 1, 5),
        ...
        "снижение_температуры_при_правильных_действиях": (lambda: sc.lower_temp(10.25), lambda: sc.temp > 60 and 
                                                                                        ...
                                                                                        elems['загб'].state == 0, 5),
        "возврат_сс_в_норму": (lambda: elems['сс'].set_state(0, lambda: sc.check('Т < Ткрит', lambda: sc.temp < sc.crit_t)), lambda: elems['сс'].state == 1, 5),
    }
